...
static void test_vector_push_and_get(void **state) {
    ...
    assert_int_equal(*(int*)vector_get(v, 0), a);
    assert_int_equal(*(int*)vector_get(v, 1), b);
    assert_int_equal(*(int*)vector_get(v, 2), c);
...
static void test_vector_set_grows(void **state) {
    ...
    vector v = vector_init(sizeof (int), 1);
    ...
    assert_int_equal(*(int*)vector_get(v, 5), x);

    // Everything in between should be NULL
    for (size_t i = 0; i < 5; i++) {
        assert_int_equal(*(int*)vector_get(v, i), 0);
    }
...
static void test_vector_out_of_bounds_get(void **state) {
    (void) state;

    vector v = vector_init(sizeof (int), 0);

    assert_null(vector_get(v, 0));
    assert_null(vector_get(v, 100));
...
static void test_vector_realloc(void **state) {
    ...
    // Check the values.
    for (size_t i = 0; i < 64; i++) {
        assert_int_equal(*(int*)vector_get(v, i), i*10);
    }
...
static void test_vector_store_structs(void **state) {
    ...
    vector v = vector_init(sizeof(Item), 0);
    ...
    Item *ra = vector_get(v, 0);
    Item *rb = vector_get(v, 1);
    Item *rc = vector_get(v, 2);
...
static void test_vector_structs_with_growth(void **state) {
    ...
    vector v = vector_init(sizeof(Item), 2);
...
int main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_vector_out_of_bounds_get),
